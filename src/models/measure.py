...
class Measure():
    ...
    def gen(self, WEIGHTS, chords, key, note_count, note_bar, bars):
        ...
        while beats_left > 0:
            s = Symbol()
            beats_left -= convert_to_float(s.gen(WEIGHTS,
                                                 note_weights, beats_left))
            self.append(s)
    ...
    def dist(self, other):
        """Returns the distance (or cost)
           from one measure to another"""
        symbol_dist = sum([symbol1.dist(symbol2) for (
            symbol1, symbol2) in zip(self.symbols, other.symbols)])
        len_dist = 2 * abs(len(self.symbols) - len(other.symbols))
        return pitch_dist(self.chord, other.chord) + symbol_dist + len_dist

    def lock_measure(self, other):
        """Locks measure if it is equal to other"""
        if self.dist(other) == 0:
            self.locked = True
        # [symbol.lock_symbol(other_symbol) for (symbol, other_symbol)
        # in zip(self.symbols, other.symbols)]
        ...
        for _ in range(count):
            ...
            if random.randint(1, 100) < 50:
                ...
                beats_left -= convert_to_float(
                    self.symbols[i - 1].mutate(WEIGHTS,
                                               note_weights,
                                               beats_left))
