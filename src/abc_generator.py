...
def run():
    ...
    # load variables
    title = args.title else 'evolved'
    generation_n = args.number else 10
    ...
    # Save song
    with open('generated_songs/{}.abc'.format(name), 'w') as file:
        file.write(best)
...
# If run with python abc_generator.py, run a profile and sort by cumulative time
